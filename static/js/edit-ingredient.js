...
document.getElementById("cancelLink").href = `/admin/ingredient/details/${ingredientId}`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add db search for ingredient name
    document.getElementById("ingredientName").addEventListener("input", async function() {
        const query = this.value.trim().replace(/\s+/g, ' ').toLowerCase();
        ...
        try {
            const res = await fetch(`/ingredients/api/editIngredients/search?ingId=${ingredientId}&q=${encodeURIComponent(query)}`, {
                method: "GET",
                headers: {
            ...
            const data = await res.json();
            if (data.length === 0) {
                existingIngredients.value = "No matching ingredients name found. Can use this.";
                return;
            }
    ...
    document.getElementById("saveIngredient").addEventListener("click", async () => {
        ...
        try {
            const response = await fetch(`/ingredients/api/edit-ingredient/${ingredientId}`, {
                method: "PUT",
                headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
            ...
            if (!response.ok) {
                errorBox.textContent = data.error || "Something went wrong while fetch edit-ingredient.";
