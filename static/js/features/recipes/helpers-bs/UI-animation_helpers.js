...
export function handleRowChange(row, event) {
  if (row.classList.contains("ingredient-row")) {
    ...
    const isNextRowComponent =
      rows[currentRowIndex + 1] &&
      rows[currentRowIndex + 1].classList.contains("component-row");
    const lastRow = rows[rows.length - 1];
    const isCurrentRowLastRow = currentRow === lastRow;
    const targetRow =
      isCurrentRowLastRow || isNextRowComponent ? currentRow : null;
    ...
    if (targetRow && currentRowInputs.some((v) => v !== "")) {
      ...
      initializeIngredientRow(newRow, token);
      updateMoveButtons();
      document.getElementById(`add-component-btn`).style.display =
        "inline-block";
    }
  }
}
...
export function updateMoveButtons() {
  ...
  rows.forEach((row, i) => {
    ...
    if (
      nextRow &&
      nextRow.classList.contains("component-row") &&
      i + 1 != rows.length - 1
    ) {
      upBtn.style.display = "none";
      downBtn.style.display = "none";
    ...
    if (
      nextRow &&
      nextRow.classList.contains("ingredient-row") &&
      i + 1 != rows.length - 1
    ) {
