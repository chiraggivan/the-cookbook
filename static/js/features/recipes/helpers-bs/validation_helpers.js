...
export function validateStepRowss() {
...
export function getRecipePayload(originalRecipeData, completeRecipeData) {
  ...
  // --- Steps Removal (always include) ---
  payload.remove_steps = completeRecipeData.steps.remove_steps;

  // --- Steps additions ---
  payload.add_steps = completeRecipeData.steps.add_steps;

  // --- Steps Updates (check with original step data) ---
  payload.update_steps = completeRecipeData.steps.update_steps
    .map((updatedRow) => {
      const originalRow = originalRecipeData.steps.find(
        (comp) => comp.procedure_id === updatedRow.procedure_id,
      );

      if (!originalRow) return updatedRow; // this line idealy should never run

      const changes = {
        procedure_id: updatedRow.procedure_id,
      };

      // CASE 1: step text changed
      if (updatedRow.step_text !== originalRow.step_text) {
        changes.component_text = updatedRow.component_text;
      }
      // CASE 2: display order changed
      if (updatedRow.step_order !== originalRow.step_order) {
        changes.step_order = updatedRow.step_order;
      }
      // FUTURE CASE 3: estimate time changed
